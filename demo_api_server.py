...
CORS(app, resources={r"/api/*": {"origins": re.compile(r"^https://venue-voice-[\w-]+\.run\.app$")}})
...
# GCS for drafts
GCS_BUCKET_NAME = 'venue-voice-drafts'
GCS_IMAGES_BUCKET = 'venue-voice-images'
gcs_client = None
try:
...
# ============================================================================
# ROUTES - GOOGLE DOCS EXPORT & EMAIL
# ============================================================================
...
def export_to_docs():
    ...
    try:
        ...
        data = request.json
        content = data.get('content', {})
        title = data.get('title', f"Venue Voice ({datetime.now().strftime('%B')}, {datetime.now().year})")
                    ...
                    for recipient in recipients:
                        try:
                            ...
                            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                                <h2 style="color: #008181;">Venue Voice Newsletter Ready for Review</h2>
                                <p>Hello,</p>
                                <p>The <strong>{month} {year}</strong> Venue Voice Newsletter has been exported to Google Docs and is ready for your review.</p>
                                ...
                                <p style="color: #999; font-size: 12px;">Sent by Venue Voice Newsletter Generator</p>
                            </div>
                            """
                            ...
                            message = Mail(
                                ...
                                to_emails=recipient,
                                subject=f"Venue Voice Newsletter ({month} {year}) - Ready for Review",
    ...
    try:
        ...
        for _attempt in range(4):
            ...
            if any(e.get('url') == article['url'] for e in entries):
                # Already recorded; no write needed.
                break
...
def publish_draft():
    ...
    try:
        ...
        dest_name = filename.replace('drafts/', 'published/', 1)
        bucket.copy_blob(source_blob, bucket, dest_name)
        # Delete original
        source_blob.delete()
        safe_print(f"[DRAFT PUBLISH] Moved {filename} -> {dest_name}")
...
def delete_draft():
    ...
    try:
        ...
        if not filename:
            return jsonify({'success': False, 'error': 'No file specified'}), 400
        if '..' in filename or not (filename.startswith('drafts/') or filename.startswith('published/')):
...
def track_selection():
    ...
    try:
        ...
        blob_name = f'selection-history/venue/{year_month}.jsonl'
